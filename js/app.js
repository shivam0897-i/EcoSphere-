...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const CHAT_INTENTS = [
    {
      ...
      reply: (state) =>
        `Your transit score is ${round1(state.scores.transitScore)} tons. You can reduce it by clicking the Public Transit action card below.`,
    },
    {
      patterns: [
        /energy|power|electric|heat|cool|thermostat|solar|renew|grid/i,
      ],
      reply: (state) =>
        `Your energy score is ${round1(state.scores.energyScore)} tons. Try switching to renewable energy or using a smart thermostat to lower it.`,
    },
    {
      patterns: [/diet|food|meat|vegan|vegetarian|plant|eat/i],
      reply: (state) =>
        `Your diet score is ${round1(state.scores.dietScore)} tons. Reducing red meat or enabling Meatless Mondays helps!`,
    },
    {
      patterns: [/dome|green.*house|plant|sapling|tree/i],
      reply: () =>
        `The green greenhouse dome represents your environmental health. Clean scores make saplings grow, while heavy scores wither them.`,
    },
    {
      patterns: [/balloon|float|rise|sink|pop|click/i],
      reply: () =>
        `Each balloon represents a carbon category (Diet, Energy, Transit). They rise when emissions are low and sink when high. Click a balloon to activate its linked action card — it'll "pop" and reduce that category's score. The position updates in real-time as your scores change!`,
    },
    {
      patterns: [/score|footprint|total|carbon|co2|emission|ton/i],
      reply: (state) =>
        `Your total carbon footprint is ${round1(state.scores.totalScore)} tons. Swapping actions and sliders will live-update the environment.`,
    },
    {
      patterns: [
        /reduc|lower|improv|help|tip|suggest|advice|how.*can|what.*do|save/i,
      ],
      reply: (state) => {
        const { dietScore, energyScore, transitScore } = state.scores;
        const highest = Math.max(dietScore, energyScore, transitScore);
        if (highest === transitScore)
          return `Your biggest impact area is Transit (${round1(transitScore)}t). Enable the Public Transit card for an instant -40% reduction, or reduce commute distance with the slider.`;
        if (highest === energyScore)
          return `Energy is your top contributor (${round1(energyScore)}t). The Renewable Energy switch (-50%) and Smart Thermostat (-15%) stack cumulatively for up to -57.5% reduction!`;
    ...
    },
    {
      ...
      reply: (state) =>
        `Your footprint: ${round1(state.scores.totalScore)}t CO₂e/year.\n• World average: ${WORLD_AVG}t\n• US average: ${US_AVG}t\n• India average: 1.9t\n• EU average: 6.8t\nYou're at ${((state.scores.totalScore / WORLD_AVG) * 100).toFixed(0)}% of the global average.`,
    },
    {
      patterns: [/reset|start.*over|clear|default/i],
      reply: () =>
        `Click the "Reset" button in the top-right corner to restore all inputs and action cards to their default values. Your chat history will also be cleared.`,
    },
    {
      patterns: [/theme|dark|light|mode|color/i],
      reply: () =>
        `Click the sun/moon icon (⚙️) in the header to toggle between dark and light themes. Your preference is saved automatically!`,
    },
    {
      patterns: [/hello|hi|hey|greet|welcome/i],
      reply: () =>
        `Hey there! 👋 I'm your EcoSphere assistant. Ask me about your carbon score, how to reduce it, what the balloons mean, or how you compare to global averages.`,
    },
  ];
  ...
  function renderActionCards(state) {
    ...
    Object.entries(actionCards).forEach(([id, key]) => {
      ...
      if (card) {
        card.setAttribute(
          "aria-checked",
          state.oneClickActions[key] ? "true" : "false",
        );
      }
    });
  }
  ...
  function renderScores(state) {
    ...
    if (totalScoreEl)
      totalScoreEl.textContent = round1(state.scores.totalScore);
    ...
    if (energyScoreEl)
      energyScoreEl.textContent = round1(state.scores.energyScore);

    const transitScoreEl = document.getElementById("transit-score");
    if (transitScoreEl)
      transitScoreEl.textContent = round1(state.scores.transitScore);
  }
